...
#[tokio::test]
async fn invalid_newsletter_returns_400() {
    let app = spawn_app().await;
    let test_cases = vec![
        (
            serde_json::json!({
    ...
    for (invalid_body, error_message) in test_cases {
        ...
        assert_eq!(
            ...
            "The API did not fail with 400 with invalid data: {}",
            error_message
        )
    }
}
...
#[tokio::test]
async fn reject_non_existing_user() {
    let app = spawn_app().await;
...
#[tokio::test]
async fn reject_invalid_password() {
    let app = spawn_app().await;
